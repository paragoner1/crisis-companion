...
impl RoleDetector {
    ...
    pub async fn detect_role(
        ...
        if let Some(profile) = &self.user_profile {
            if let Some(role) = self.check_user_profile(profile, emergency_context) {
                return Ok(RoleDetectionResult {
                    ...
                    role: role.clone(),
            detected_role: role.clone(),
                    method: RoleDetectionMethod::UserProfile,
                    confidence: 0.9,
                    detection_time_ms: start_time.elapsed().as_millis() as u64,
                    context_data: Some(RoleContext {
                        confidence: 0.9,
                        current_detected_role: UserRole::Unknown,
                        current_role: UserRole::Unknown,
                        detection_method: RoleDetectionMethod::UserProfile,
                        last_updated: chrono::Utc::now().timestamp() as u64,
                        user_id: "".to_string(),
                        phrase_detected: Some(emergency_context.phrase.clone()),
                        sensor_data: emergency_context.sensor_data.clone(),
                        user_profile: Some(profile.clone()),
                        emergency_type: Some(emergency_context.emergency_type.clone()),
                    }),
                });
            }
        }
        ...
        if ai_result.confidence >= self.config.ai_confidence_threshold {
            return Ok(RoleDetectionResult {
                    timestamp: chrono::Utc::now().timestamp() as u64,
                role: ai_result.role.clone(),
                detected_role: ai_result.role.clone(),
    ...
    async fn multi_modal_confirmation(
        ...
        match ui_result {
            ...
            Err(_) => {
                ...
                return Ok(RoleDetectionResult {
                    timestamp: chrono::Utc::now().timestamp() as u64,
                    role: voice_result.role.clone(),
                    detected_role: voice_result.role.clone(),
...
impl AIInference {
    ...
    fn analyze_sensor_data(&self, sensor_data: &SensorData) -> Option<SensorAnalysis> {
        ...
        match sensor_data.device_movement {
            Some(DeviceMovement::Stationary) => {
                // Stationary device suggests bystander (helping someone)
                confidence += 0.7;
                role = UserRole::Bystander;
            }
            Some(DeviceMovement::Swimming) | Some(DeviceMovement::Falling) => {
                // Swimming/falling suggests victim
                confidence += 0.8;
                role = UserRole::Victim;
            }
            Some(DeviceMovement::Running) => {
...
impl SensorFusion {
    ...
    pub async fn analyze_sensors(
        ...
        let movement_result = self.movement_analyzer.analyze(sensor_data.device_movement.as_ref().unwrap());
        let location_result = self.location_analyzer.analyze(&sensor_data.location_context);
        let audio_result = self.audio_analyzer.analyze(sensor_data.audio_environment.as_ref().unwrap());
    ...
    pub fn get_insights(&self) -> LearningInsights {
        ...
        LearningInsights {
            ...
            average_confidence: (self.historical_data.iter()
                ...
                .sum::<f64>() / total_examples as f64) as f32,
        }
    }
}
...
impl EmergencyContext {
    ...
    pub fn to_role_context(&self) -> RoleContext {
        RoleContext {
            ...
            current_detected_role: UserRole::Unknown,
            current_role: UserRole::Unknown,
            detection_method: RoleDetectionMethod::VoiceConfirmation,
            last_updated: chrono::Utc::now().timestamp() as u64,
            user_id: "".to_string(),
            phrase_detected: Some(self.phrase.clone()),
            sensor_data: self.sensor_data.clone(),
            user_profile: None, // Will be set by caller
            emergency_type: Some(self.emergency_type.clone()),
        }
    }
}
...
impl MovementAnalyzer {
    ...
    pub fn analyze(&self, _movement: &DeviceMovement) -> SensorAnalysis {
        SensorAnalysis {
            detected_role: UserRole::Unknown,
            role: UserRole::Unknown,
