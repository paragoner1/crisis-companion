...
/// Role types for emergency response
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum RoleType {
    /// Bystander role
    Bystander,
...
impl RoleType {
    ...
    pub fn display_name(&self) -> &'static str {
        match self {
            RoleType::Bystander => "Bystander",
            RoleType::Caregiver => "Caregiver",
            RoleType::TrainedResponder => "Trained Responder",
            RoleType::MedicalProfessional => "Medical Professional",
        }
    }
}

/// Location types for context analysis
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ActionType {
    ...
    pub fn display_name(&self) -> &'static str {
        match self {
            ActionType::ExtractedVictim => "Extracted Victim",
            ActionType::StartedCPR => "Started CPR",
            ActionType::Called911 => "Called 911",
            ActionType::AppliedFirstAid => "Applied First Aid",
            ActionType::UsedAED => "Used AED",
            ActionType::AdministeredMedication => "Administered Medication",
            ActionType::StabilizedVictim => "Stabilized Victim",
            ActionType::TransportedVictim => "Transported Victim",
        }
    }
} 