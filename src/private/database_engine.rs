    ...
    pub fn get_emergency_instructions(&self, emergency_type: &EmergencyType) -> AppResult<Vec<EmergencyInstruction>> {
        ...
        let instructions = stmt.query_map(params![emergency_type_id], |row| {
            Ok(EmergencyInstruction {
                id: Uuid::parse_str(&row.get::<_, String>(0)?).unwrap().to_string(),
                emergency_type: emergency_type.clone(),
                step_number: row.get(2)?,
                title: row.get(3)?,
                description: row.get(4)?,
                audio_file: row.get(5)?,
                instruction: row.get(4)?, // Use description as instruction
                duration: Some(row.get(6)?), // Assuming duration is in column 6
    ...
    pub fn record_emergency_response(&self, response: &EmergencyResponse) -> AppResult<()> {
        ...
        conn.execute(
            ...
            params![
                ...
                emergency_type_id,
                chrono::DateTime::from_timestamp(response.trigger_timestamp as i64, 0).unwrap().format("%Y-%m-%d %H:%M:%S").to_string(),
                chrono::DateTime::from_timestamp(response.response_start as i64, 0).unwrap().format("%Y-%m-%d %H:%M:%S").to_string(),
                response.response_end.map(|t| chrono::DateTime::from_timestamp(t as i64, 0).unwrap().format("%Y-%m-%d %H:%M:%S").to_string()),
