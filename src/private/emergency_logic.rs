...
impl EmergencyHandler {
    ...
    pub async fn start_emergency_response(&self, emergency_type: EmergencyType) -> AppResult<EmergencyResponse> {
        ...
        let response = EmergencyResponse {
            ...
            location: Location { latitude: 0.0, longitude: 0.0, altitude: None, accuracy: None, timestamp: 0 },
            timestamp: chrono::Utc::now().timestamp() as u64,
            instructions: vec![],
            current_step: 0,
            completed_steps: vec![],
            estimated_completion_time: None,
            emergency_contacts_notified: vec![],
            emergency_services_contacted: false,
            id: Uuid::new_v4().to_string(),
            emergency_type: emergency_type.clone(),
            trigger_timestamp: chrono::Utc::now().timestamp() as u64,
            response_start: chrono::Utc::now().timestamp() as u64,
    ...
    pub async fn complete_emergency_response(&self) -> AppResult<()> {
        ...
        // Update response status
        {
            ...
            if let Some(ref mut response) = *current {
                response.response_end = Some(chrono::Utc::now().timestamp() as u64);
        ...
        match emergency_type {
            EmergencyType::Drowning => Ok(vec![
                EmergencyInstruction {
                    duration: Some(60),
                    instruction: "Follow emergency protocol".to_string(),
